...
const getAllMaps = function() {
  return db.query(`
  SELECT id, title, city FROM maps
  WHERE isPublic=true
  `)
  .then(data => data.rows)
};
...
return {getIdByUserName, getAllLocations, getAllMaps, getMapIdbyUserId, getMapsbyId, getUsers, newUser, newMap, newPoint, newLike, getFavouritesById, search, findUserIdByName, getPassword, isRegisteredBefore}
